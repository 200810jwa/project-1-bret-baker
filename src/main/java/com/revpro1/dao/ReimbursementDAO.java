...
//		}
	...
	public boolean approveById(int id, int resolverId) {
		...
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			String sql = "UPDATE project1.ers_reimbursement SET reimb_resolved = CURRENT_TIMESTAMP, reimb_resolver = ?, reimb_status_id = 2 WHERE reimb_id = ?";
	...
	public boolean denyById(int id, int resolverId) {
		...
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			String sql = "UPDATE project1.ers_reimbursement SET reimb_resolved = CURRENT_TIMESTAMP, reimb_resolver = ?, reimb_status_id = 3 WHERE reimb_id = ?";
	...
	public List<Reimbursement> getByStatusId(int statusId) {
		...
		try (Connection conn = ConnectionUtil.getConnection()) {
			...
			while (rs.next()) {
				...
				String author = rs.getString("reimb_author");
				String resolver = rs.getString("reimb_resolver");
				int statusId1 = rs.getInt("reimb_status_id");
				int typeId = rs.getInt("reimb_type_id");
				Reimbursement reimb = new Reimbursement(id1, amount, submitted, resolved, description, receipt, author,
						resolver, statusId1, typeId);
				reimbs.add(reimb);
			}

		} catch (SQLException e) {
			e.printStackTrace();
			System.out.println("WE FAILED TO RETRIEVE REIMBURSEMENTS BY STATUS");
			return null;
		} finally {
			closeResources();
		}
